...
describe('Misc', () => {
  ...
  describe('GET /', () => {
    it('<200> should always return with the API server information', async () => {
      const res = await request.get('/').expect('Content-Type', /json/).expect(200);
  ...
  describe('GET /spec', () => {
    it('<200> should always return API specification in swagger format', async () => {
      const res = await request.get('/spec').expect('Content-Type', /json/).expect(200);
  ...
  describe('GET /status', () => {
    it('<200> should return `healthy` status if all components are healthy', async () => {
      const res = await request.get('/status').expect('Content-Type', /json/).expect(200);
