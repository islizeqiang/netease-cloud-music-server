...
const aesEncrypt = (buffer, mode, key, iva) => {
  const cipher = crypto.createCipheriv(`aes-128-${mode}`, key, iva);
  return Buffer.concat([cipher.update(buffer), cipher.final()]);
};

const rsaEncrypt = (buffer, key) => {
  const newBuffer = Buffer.concat([Buffer.alloc(128 - buffer.length), buffer]);
  return crypto.publicEncrypt({ key, padding: crypto.constants.RSA_NO_PADDING }, newBuffer);
};
